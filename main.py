...
DRIVE_ROOT_FOLDER = "TikTokRecordings"
...
def try_init_drive():
    global drive_service, uploaders
    try:
        ...
        if drive_service:
            ...
            for u in usernames:
                uploaders[u] = GoogleDriveUploader(
                    drive_service, drive_folder_root=DRIVE_ROOT_FOLDER
                )
        else:
            logger.warning("Drive service unavailable (no credentials yet)")
    except Exception as e:
        logger.exception("Drive init failed: %s", e)
                ...
                if is_live:
                    ...
                    if rec and rec.is_running():
                        ...
                        if out_file and os.path.exists(out_file) and username in uploaders:
                            try:
                                uploaders[username].upload_file(
                                    out_file, remote_subfolder=username
                                )
                            except Exception as e:
